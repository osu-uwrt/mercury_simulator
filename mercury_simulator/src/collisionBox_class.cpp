...
collisionBox::collisionBox(
  std::string name_, double length_, double width_, double height_, v3d baseCenter, v3d baseOffset_,
  quat baseOrientation, quat baseOrientationOffset_)
{
  name = name_;
  length = abs(length_);
  width = abs(width_);
...
v3d collisionBox::getCenter() { return center; }
v3d collisionBox::getAxis(int index) { return rotM.col(index); }
std::string collisionBox::getName() { return name; }
m3d collisionBox::rotationMatrix() { return rotM; }
...
bool collisionBox::isInBox(const v3d & point)
{
  ...
  return abs(point_body[0]) <= length / 2 && abs(point_body[1]) <= width / 2 &&
...
double collisionBox::getLength() { return length; }
double collisionBox::getWidth() { return width; }
double collisionBox::getHeight() { return height; }
quat collisionBox::getOrientation() { return orientation; }
