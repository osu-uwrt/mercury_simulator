...
bool Robot::loadParams(rclcpp::Node::SharedPtr Node) {
    ...
    if (node->get_parameter("vehicle_config_path", vehicle_config_file_path) &&
        ...
        try {
            ...
            YAML::Node controller_config =
                YAML::LoadFile(vehicle_config_file_path + "_controller.yaml");
            YAML::Node thruster_config =
                YAML::LoadFile(vehicle_config_file_path + "_xacro_frames.yaml");
            YAML::Node simulator_config = YAML::LoadFile(simulator_config_file);
            Robot::storeConfigData(
                vehicle_config, controller_config, simulator_config, thruster_config);

            // Loading params successeeded
            return true;
        } catch (const YAML::Exception & e) {
            RCLCPP_ERROR(
                node->get_logger(), "Failed to parse the config file: %s, line %d, col %d, pos %d",
                e.what(), e.mark.line, e.mark.column, e.mark.pos);
 ...
 */
void Robot::storeConfigData(
    const YAML::Node & vehicle_config, const YAML::Node & controller_config,
    const YAML::Node & simulator_config, const YAML::Node & thruster_config) {
    ...
    std::vector<double> bodyInertiaArray = getYamlNodeAs<std::vector<double>>(
        simulator_config, {"vehicle_properties", name, "inertia3x3"});
    m3d bodyInertia;
    bodyInertia << bodyInertiaArray[0], bodyInertiaArray[1], bodyInertiaArray[2],
        bodyInertiaArray[3], bodyInertiaArray[4], bodyInertiaArray[5], bodyInertiaArray[6],
        bodyInertiaArray[7], bodyInertiaArray[8];
    invBodyInertia = bodyInertia.inverse();

    try {
        // Get bouyancy vector from feed forward
        std::vector<double> feedForward = getYamlNodeAs<std::vector<double>>(
            simulator_config, {"vehicle_properties", name, "base_wrench"});

        bouyancyVector = v3d(0, 0, -feedForward[2]) - weightVector;
        // Calculate COB from feed forward: r = T/F
        r_cob = v3d(
            feedForward[4] / bouyancyVector.norm(), -feedForward[3] / bouyancyVector.norm(), 0.005);
    } catch (std::runtime_error & e) {
    ...
    r_cod = std2v3d(
                getYamlNodeAs<std::vector<double>>(
                    simulator_config, {"vehicle_properties", name, "cod"})) -
            r_com;
    ...
    try {
        ...
        ballast_in_flow_rate =
            getYamlNodeAs<double>(vehicle_config, {"active_ballast", "in_flow_rate"});
        ballast_out_flow_rate =
            getYamlNodeAs<double>(vehicle_config, {"active_ballast", "out_flow_rate"});
        ...
        RCLCPP_INFO(
            node->get_logger(),
            "Could not parse all active ballast settings (%s). Continuing with active ballast "
            "disabled",
            e.what());
    }
    ...
    imu_enabled = false;
    try {
        std::vector<double> imu_pose = getYamlNodeAs<std::vector<double>>(
            simulator_config, {"vehicle_properties", name, "imu", "pose"});
        r_imu = v3d(imu_pose[0], imu_pose[1], imu_pose[2]) - r_com;
        q_imu = rpy2quat(imu_pose[3], imu_pose[4], imu_pose[5]);
        imu_rate = 1.0 / getYamlNodeAs<double>(
                             simulator_config, {"vehicle_properties", name, "imu", "rate"});
        imu_yawDrift = getYamlNodeAs<double>(
            simulator_config, {"vehicle_properties", name, "imu", "yaw_drift"});
        imu_sigmaAccel = getYamlNodeAs<double>(
            simulator_config, {"vehicle_properties", name, "imu", "sigma_accel"});
        imu_sigmaOmega = getYamlNodeAs<double>(
                             simulator_config, {"vehicle_properties", name, "imu", "sigma_omega"}) *
                         M_PI / 180;
        imu_sigmaAngle = getYamlNodeAs<double>(
                             simulator_config, {"vehicle_properties", name, "imu", "sigma_angle"}) *
                         M_PI / 180;
        ...
        RCLCPP_INFO(
            node->get_logger(),
            "Could not parse all IMU Settings (%s). Continuing with IMU disabled", e.what());
    }
    ...
    try {
        r_depth = std2v3d(
                      getYamlNodeAs<std::vector<double>>(
                          simulator_config, {"vehicle_properties", name, "depth", "pose"})) -
                  r_com;
        depth_rate = 1.0 / getYamlNodeAs<double>(
                               simulator_config, {"vehicle_properties", name, "depth", "rate"});
        depth_sigma =
            getYamlNodeAs<double>(simulator_config, {"vehicle_properties", name, "depth", "sigma"});
        ...
        RCLCPP_INFO(
            node->get_logger(),
            "Could not parse all depth sensor settings (%s). Continuing with depth disabled",
            e.what());
    }
    ...
    dvl_enabled = false;
    try {
        std::vector<double> dvl_pose = getYamlNodeAs<std::vector<double>>(
            simulator_config, {"vehicle_properties", name, "dvl", "pose"});
        r_dvl = v3d(dvl_pose[0], dvl_pose[1], dvl_pose[2]) - r_com;
        q_dvl = rpy2quat(dvl_pose[3], dvl_pose[4], dvl_pose[5]);
        dvl_rate = 1.0 / getYamlNodeAs<double>(
                             simulator_config, {"vehicle_properties", name, "dvl", "rate"});
        dvl_sigma =
            getYamlNodeAs<double>(simulator_config, {"vehicle_properties", name, "dvl", "sigma"});
        ...
        RCLCPP_INFO(
            node->get_logger(),
            "Could not parse all DVL sensor settings (%s). Continuing with DVL disabled", e.what());
    }

    // Drag information
    dragCoef = getYamlNodeAs<std::vector<double>>(
        simulator_config, {"vehicle_properties", name, "damping"});
