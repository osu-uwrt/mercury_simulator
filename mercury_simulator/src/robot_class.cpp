...
void Robot::storeConfigData(const YAML::Node & vehicle_config, const YAML::Node & simulator_config)
{
  ...
  try {
    d_com = std2v3d(
      getYamlNodeAs<std::vector<double>>(simulator_config, {"vehicle_properties", name, "dcom"}));
    ...
    RCLCPP_INFO(
      node->get_logger(), "Failed to load dcom/env force. Center of mass discrepancy is disabled!");
  }
  ...
  m3d bodyInertia;
  bodyInertia << bodyInertiaArray[0], bodyInertiaArray[1], bodyInertiaArray[2], bodyInertiaArray[3],
    bodyInertiaArray[4], bodyInertiaArray[5], bodyInertiaArray[6], bodyInertiaArray[7],
    bodyInertiaArray[8];
  ...
  try {
    ...
    RCLCPP_INFO(
      node->get_logger(),
      "Could not parse all active ballast settings (%s). Continuing with active ballast disabled",
      e.what());
  }
  ...
  try {
    ...
    RCLCPP_INFO(
      node->get_logger(), "Could not parse all IMU Settings (%s). Continuing with IMU disabled",
      e.what());
  }
  ...
  try {
    ...
    RCLCPP_INFO(
      node->get_logger(),
      "Could not parse all depth sensor settings (%s). Continuing with depth disabled", e.what());
  }
  ...
  if (ACOUSTIC_DATA) {
    try {
      ...
      std::vector<double> fakePingerPose =
        getYamlNodeAs<std::vector<double>>(simulator_config, {"acoustics", "fake_pinger", "pose"});
      ...
      RCLCPP_INFO(
        node->get_logger(),
        "Could not parse acoustics information(%s). Continuing with acoustics disabled.", e.what());
    }
  } else {
...
v3d Robot::calcDragForces(const v3d & linVel, const double & depth)
{
  ...
  double dragX =
    -dragCoef[2] + dragCoef[1] * abs(linVel[0]) + dragCoef[2] * exp(abs(linVel[0]) / dragCoef[3]);
  double dragY =
    -dragCoef[6] + dragCoef[5] * abs(linVel[1]) + dragCoef[6] * exp(abs(linVel[1]) / dragCoef[7]);
  ...
  for (auto ti = claw_objects.begin(); ti != claw_objects.end(); ti++) {
    if (!strcmp(object_name_msg.data.c_str(), ti->first.c_str())) {
      ...
      v3d claw_translation =
        v3d(claw_translation_vector.x, claw_translation_vector.y, claw_translation_vector.z);
