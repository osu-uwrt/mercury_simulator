...
#include "mercury_simulator/RobotClass.h"
#include "mercury_simulator/collisionBox.h"
#include "mercury_simulator/settings.h"
...
public:
    ...
    PhysicsSimNode() : Node("physics_simulator") {
        ...
        dvlPub =
            this->create_publisher<geometry_msgs::msg::TwistWithCovarianceStamped>("dvl_twist", 10);
        depthPub =
            this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("depth/pose", 10);
        acousticsPub =
            this->create_publisher<geometry_msgs::msg::Vector3Stamped>("acoustics/delta_t", 10);
        collisionBoxPub = this->create_publisher<visualization_msgs::msg::MarkerArray>(
            "simulator/collisionMarkers", 10);
        thrusterTelemetryPub = this->create_publisher<mercury_msgs::msg::DshotPartialTelemetry>(
            "state/thrusters/telemetry", 10);
        exhaustSolenoidPub =
            this->create_publisher<std_msgs::msg::Bool>("state/solenoid/exhaust", 10);
        pressureSolenoidPub =
            this->create_publisher<std_msgs::msg::Bool>("state/solenoid/pressure", 10);
        ...
        clawObjectSub = this->create_subscription<std_msgs::msg::String>(
            "simulator/loaded_claw_object", 10,
            std::bind(&PhysicsSimNode::setLoadedClawObjectCallback, this, _1));
        softwareKillSub = this->create_subscription<mercury_msgs::msg::KillSwitchReport>(
            "command/software_kill", 10, std::bind(&PhysicsSimNode::killSwitchCallback, this, _1));
        exhaustSolenoidSub = this->create_subscription<std_msgs::msg::Bool>(
            "command/solenoid/exhaust", 10, std::bind(&PhysicsSimNode::exhaustSubCb, this, _1));
        pressureSolenoidSub = this->create_subscription<std_msgs::msg::Bool>(
            "command/solenoid/pressure", 10, std::bind(&PhysicsSimNode::pressureSubCb, this, _1));
        waterSolenoidSub = this->create_subscription<std_msgs::msg::Bool>(
            "command/solenoid/water", 10, std::bind(&PhysicsSimNode::waterSubCb, this, _1));
        ...
        thrusterTelemetryTimer =
            this->create_wall_timer(0.5s, std::bind(&PhysicsSimNode::pubThrusterTelemetry, this));
        paramRefreshTimer = this->create_wall_timer(
            5.0s, std::bind(&PhysicsSimNode::refreshSimulationParameters, this));

        // Services for setting odom and simulator
        poseClient = this->create_client<robot_localization::srv::SetPose>(
            "/" + robot.getName() + "/set_pose");
    ...
    bool init() {
        ...
        if (paramsLoaded && collisionBoxesLoaded) {
            ...
            RCLCPP_INFO(
                this->get_logger(), "Initization successful. Timers created to fake sensor data");
            imuTimer = this->create_wall_timer(
                imuRate, std::bind(&PhysicsSimNode::publishFakeIMUData, this));
            dvlTimer = this->create_wall_timer(
                dvlRate, std::bind(&PhysicsSimNode::publishFakeDVLData, this));
            depthTimer = this->create_wall_timer(
                depthRate, std::bind(&PhysicsSimNode::publishFakeDepthData, this));
            acousticsTimer = this->create_wall_timer(
                2s, std::bind(&PhysicsSimNode::publishFakeAcousticsData, this));
        } else {
            // Report error
            if (!paramsLoaded)
                RCLCPP_INFO(
                    this->get_logger(),
                    "Failed to read YAML parameter data, loaded params: %s, loaded collision: %s",
    ...
    /**
     * @brief Solves system of ODEs representing robot physics using fourth order Runge-Kutta
     * numerical method. This function is runs in an infinite loop to run the simulator, which also
     * spins the ROS node
     *
    ...
    void rungeKutta4() {
        ...
        while (rclcpp::ok()) {
            ...
            if (COLLISION_TOGGLE) state = handleCollisions(state);
...
private:
    ...
    v3d calcForces(const quat & q, const v3d & linVel, const double & depth) {
        // Some forces are easier to calculate in the body frame, like thruster forces and drag
        // forces, doing those first Linear velocity is converted to body frame before passing it to
        // drag force function

        v3d body_forces =
            robot.getThrusterForces() + robot.calcDragForces(q.conjugate() * linVel, depth);
        v3d world_forces =
            q * body_forces + robot.getNetBouyantForce(depth) + robot.getClawObjectForces();
    ...
    v3d calcTorques(const quat & q, const v3d & linVel, const v3d & angVel, const double & depth) {
        // Angular and linear velocity is converted to body frame before passing it to drag torque
        // function
        v3d body_torques =
            robot.getThrusterTorques() +
            robot.calcDragTorques(q.conjugate() * linVel, q.conjugate() * angVel, depth) +
            robot.getClawObjectTorques();
        ...
        for (collisionBox & robotBox : robotBoxes) {
            ...
            for (collisionBox obstacleBox : obstacleBoxes) {
                ...
                if (collision.collided) {
                    RCLCPP_DEBUG(
                        this->get_logger(), "%s has collided with %s", robot.getName().c_str(),
                    ...
                    double impulse = -(1.0 + COEF_OF_RESTITUTION) * v_rel /
                                     (1.0 / robot.getTotalMass() +
                                      (invWorldInertia * (r_a.cross(collision.unitDirection)))
                                          .cross(r_a)
                                          .dot(collision.unitDirection));
                    // Seperate the objects
                    state.segment(0, 3) =
                        collision.unitDirection * collision.depth + state.segment(0, 3);
                    // Update momentums
                    state.segment(7, 3) = state.segment(7, 3) + impulse * collision.unitDirection;
                    state.segment(10, 3) =
                        state.segment(10, 3) + r_a.cross(impulse * collision.unitDirection);
                }
            }
        }
        return state;
    ...
    /**
     * @brief Determines collision information for two collision boxes using the Seperating Axis
     * Theorem (SAT) in 3D
     ...
     * @returns Struct containing wether there was a collision, collision point, depth, and
     * collision normal
     */
    collisionResult computeCollision(collisionBox & box1, collisionBox & box2) {
        ...
        for (v3d axis : axes) {
            // If axis is zero vector, don't compute it. (Can occour because vectors from cross
            // products could be alligned)
            if (axis.norm() < 0.9) continue;
            // If the center of box1 is to the right of box2, flip axis so it becomes to the left
            if (box1.getCenter().dot(axis) > box2.getCenter().dot(axis)) axis = -axis;
        ...
        int index;
        // Ok, we have the axis that has the smallest overlap, but need to find if it belongs to
        // first or second box Need to know which box it belongs to because it determines which
        // box's vertex to use as the collision point Note that the minAxis might not be one of the
        // three axes alligned with a box edge since the cross product of the axes could be the
        // minAxes The dot product is taken with each of the box axes, the one with the largest
        // matches the box the most. This informs us on what box's vertex to use for the collision
        // point
        (boxAxes.transpose() * minAxis).cwiseAbs().maxCoeff(&index);
        if (index < 3)
            // A face of the first box is getting intersected, thus a vertex from the second box
            // must be intersecting it
            point = box2.minVertex(minAxis);
        else
            // A face of the second box is getting intersected, thus a vertex from the first box
            // must be intersecting it
        ...
        if (!box1.isInBox(point) || !box2.isInBox(point)) {
            ...
            // Shuffle the point back and forth between the two boxes to iteratively move the vertex
            // closer to the edge collision point
    ...
    bool loadCollisionFiles() {
        ...
        if (sceneFile["robot"][robot.getName()]["collision"].IsDefined()) {
            std::string robotCollisionPath =
                collisionFolder / "robots" /
                getYamlNodeAs<string>(sceneFile, {"robot", robot.getName(), "collision"});
            urdf::ModelInterfaceSharedPtr robotModel = urdf::parseURDFFile(robotCollisionPath);
            if (!robotModel) {
                RCLCPP_ERROR(
                    this->get_logger(), "Failed to load robot model: %s",
                    robotCollisionPath.c_str());
                return false;
            }
        ...
        if (sceneFile["objects"].IsDefined()) {
            for (auto const & entryPair : sceneFile["objects"]) {
                ...
                if (!entry["collision"].IsDefined()) {
                    RCLCPP_WARN(
                        this->get_logger(), "Skipping entry  %s without collision info.",
                        entryPair.first.as<string>().c_str());
                    continue;
                }
                std::string objectCollisionPath =
                    collisionFolder / "objects" / getYamlNodeAs<string>(entry, {"collision"});
                urdf::ModelInterfaceSharedPtr objectModel =
                    urdf::parseURDFFile(objectCollisionPath);
                if (!objectModel) {
                    RCLCPP_ERROR(
                        this->get_logger(), "Failed to load object model: %s, continuing...",
                        objectCollisionPath.c_str());
                    continue;
                }
                // Get position and orientation
                v3d position(
                    toDouble(entry["pose"]["x"]), toDouble(entry["pose"]["y"]),
                    toDouble(entry["pose"]["z"]));
                tf2::Quaternion tf2Quat;
                tf2Quat.setRPY(
                    deg2rad(toDouble(entry["pose"]["roll"])),
                ...
                for (collisionBox newBox : newBoxes) obstacleBoxes.push_back(newBox);
            }
        }
        // Add floor to list
    ...
    /**
     * @brief Unpacks collision information from URDF model and stores it's content into a
     * collisionBox object. Note this only works for boxes
     ...
     */
    std::vector<collisionBox> unpackURDF(
        urdf::ModelInterfaceSharedPtr model, v3d basePosition, quat baseOrientation) {
        ...
        for (const auto & link_pair : model->links_) {
            ...
            for (const auto & collision : link->collision_array) {
                ...
                // Check if it's a or not box (current collision implementation only works with
                // boxes)
                if (collision_ptr->geometry->type != urdf::Geometry::BOX) {
                    RCLCPP_ERROR(
                        this->get_logger(),
                        "Link %s uses a non-box geometry, only boxes are supportede. Skipping "
                        "element",
                        link->name.c_str());
                    continue;
                }
                const urdf::BoxSharedPtr & box =
                    std::static_pointer_cast<urdf::Box>(collision_ptr->geometry);
                ...
                v3d baseOffset(
                    collision_ptr->origin.position.x, collision_ptr->origin.position.y,
                    collision_ptr->origin.position.z);
                // Get box orientation relative to obstacle's base
                quat baseOrientationOffset(
                    collision_ptr->origin.rotation.w, collision_ptr->origin.rotation.x,
                    collision_ptr->origin.rotation.y, collision_ptr->origin.rotation.z);

                // Add box to list
                collisionBox newBox = collisionBox(
                    link->name, length, width, height, basePosition, baseOffset, baseOrientation,
    ...
    // Fabricates fake depth sensor data from robot's state and publishes it to topic. Called on a
    // timer
        ...
        if (robot.getDepthEnabled()) {
            ...
            if (!hasNans(depthMsg)) {
                ...
                RCLCPP_WARN_SKIPFIRST_THROTTLE(
                    ...
                    "NaNs present in outgoing depth message. Skipped.");
            }
        }
    }
    ...
    void publishFakeDVLData() {
        if (robot.getDVLEnabled() && robot.dvlTransformAvailable()) {
            ...
            if (!hasNans(dvlMsg)) {
                ...
                RCLCPP_WARN_SKIPFIRST_THROTTLE(
                    get_logger(), *get_clock(), 1000,
                    "NaNs present in outgoing DVL message. Skipped.");
            }
        }
    }
    ...
    void publishFakeIMUData() {
        if (robot.getIMUEnabled() && robot.imuTransformAvailable()) {
            ...
            v3d imuAccel = robot.getLatestLinAccel() +
            ...
            if (!hasNans(imuMsg)) {
                ...
                RCLCPP_WARN_SKIPFIRST_THROTTLE(
                    get_logger(), *get_clock(), 1000,
                    "NaNs present in outgoing IMU message. Skipped.");
            }
        }
    }
    ...
    void pubThrusterTelemetry() {
        // Create messages
        mercury_msgs::msg::DshotPartialTelemetry msgLow;
        mercury_msgs::msg::DshotPartialTelemetry msgHigh;
    ...
    void forceCallback(const std_msgs::msg::Float32MultiArray & thrusterForces) {
        ...
        thrusterForcesStamped commandedThrust(
            vXd::Zero(robot.getThrusterCount()), this->get_clock()->now().seconds());
        // Only set forces if robot's enabled
        if (enabled) commandedThrust.thrusterForces = convert2eigen(thrusterForces.data);
    ...
    // Mirrors software kill to firmware kill. This is done to make the enable/disable button in
    // RViz work
    void killSwitchCallback(const mercury_msgs::msg::KillSwitchReport & softwareKillMsg) {
        ...
        if (softwareKillMsg.kill_switch_id == 1) {
            ...
            if (!enabled)
                robot.addToThrusterQue(thrusterForcesStamped(
                    vXd::Zero(robot.getThrusterCount()), this->get_clock()->now().seconds()));
        }
    }

    void updateActiveBallast() { robot.updateActiveBallast(robot.getBallastState()); }
    ...
    void setSim(const std::shared_ptr<robot_localization::srv::SetPose::Request> poseRequest) {
        ...
        while (!poseClient->wait_for_service(1s)) {
            if (!rclcpp::ok()) {
                RCLCPP_ERROR(
                    this->get_logger(),
                    "Interrupted while waiting for the set simulator pose service. Exiting.");
                break;
            }
        }
        // Send request
    ...
    vXd convert2eigen(std::vector<float> stdVector) {
        Eigen::VectorXf eigenVector =
            Eigen::Map<Eigen::VectorXf>(stdVector.data(), stdVector.size());
    ...
    bool hasNans(const geometry_msgs::msg::Vector3 & v3Msg) {
        return isnan(v3Msg.x) || isnan(v3Msg.y) || isnan(v3Msg.z);
    }

    bool hasNans(const geometry_msgs::msg::Point & pMsg) {
        return isnan(pMsg.x) || isnan(pMsg.y) || isnan(pMsg.z);
    }

    bool hasNans(const geometry_msgs::msg::Quaternion & quatMsg) {
        return isnan(quatMsg.x) || isnan(quatMsg.y) || isnan(quatMsg.z) || isnan(quatMsg.w);
    }

    bool hasNans(const geometry_msgs::msg::PoseWithCovarianceStamped & poseMsg) {
        return hasNans(poseMsg.pose.covariance) || hasNans(poseMsg.pose.pose.orientation) ||
               hasNans(poseMsg.pose.pose.position);
    }

    bool hasNans(const sensor_msgs::msg::Imu & imuMsg) {
        return hasNans(imuMsg.angular_velocity) || hasNans(imuMsg.angular_velocity_covariance) ||
               hasNans(imuMsg.linear_acceleration) ||
               hasNans(imuMsg.linear_acceleration_covariance) || hasNans(imuMsg.orientation) ||
               hasNans(imuMsg.orientation_covariance);
    }

    bool hasNans(const geometry_msgs::msg::TwistWithCovarianceStamped & twistMsg) {
        return hasNans(twistMsg.twist.covariance) || hasNans(twistMsg.twist.twist.angular) ||
    ...
    rclcpp::Publisher<mercury_msgs::msg::DshotPartialTelemetry>::SharedPtr thrusterTelemetryPub;
    rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr exhaustSolenoidPub, pressureSolenoidPub,
        waterSolenoidPub;

    rclcpp::Subscription<mercury_msgs::msg::KillSwitchReport>::SharedPtr softwareKillSub;
    rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr exhaustSolenoidSub, pressureSolenoidSub,
        waterSolenoidSub;
};
