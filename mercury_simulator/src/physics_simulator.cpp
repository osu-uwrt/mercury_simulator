...
public:
  ...
  PhysicsSimNode() : Node("physics_simulator")
  {
    ...
    poseClient =
      this->create_client<robot_localization::srv::SetPose>("/" + robot.getName() + "/set_pose");
  ...
  bool init()
  {
    ...
    if (paramsLoaded && collisionBoxesLoaded) {
      ...
      imuTimer =
        this->create_wall_timer(imuRate, std::bind(&PhysicsSimNode::publishFakeIMUData, this));
      dvlTimer =
        this->create_wall_timer(dvlRate, std::bind(&PhysicsSimNode::publishFakeDVLData, this));
      depthTimer =
        this->create_wall_timer(depthRate, std::bind(&PhysicsSimNode::publishFakeDepthData, this));
      acousticsTimer =
        this->create_wall_timer(2s, std::bind(&PhysicsSimNode::publishFakeAcousticsData, this));
    } else {
      // Report error
      if (!paramsLoaded)
        RCLCPP_INFO(
...
private:
    ...
    for (collisionBox & robotBox : robotBoxes) {
      ...
      for (collisionBox obstacleBox : obstacleBoxes) {
        ...
        if (collision.collided) {
          ...
          double impulse =
            -(1.0 + COEF_OF_RESTITUTION) * v_rel /
            (1.0 / robot.getTotalMass() + (invWorldInertia * (r_a.cross(collision.unitDirection)))
                                            .cross(r_a)
  ...
  bool loadCollisionFiles()
  {
    ...
    if (sceneFile["robot"][robot.getName()]["collision"].IsDefined()) {
      ...
      if (!robotModel) {
        RCLCPP_ERROR(
          this->get_logger(), "Failed to load robot model: %s", robotCollisionPath.c_str());
        return false;
      }
    ...
    if (sceneFile["objects"].IsDefined()) {
      for (auto const & entryPair : sceneFile["objects"]) {
        ...
        v3d position(
          toDouble(entry["pose"]["x"]), toDouble(entry["pose"]["y"]), toDouble(entry["pose"]["z"]));
        tf2::Quaternion tf2Quat;
        tf2Quat.setRPY(
          deg2rad(toDouble(entry["pose"]["roll"])), deg2rad(toDouble(entry["pose"]["pitch"])),
    ...
    for (const auto & link_pair : model->links_) {
      ...
      for (const auto & collision : link->collision_array) {
        ...
        if (collision_ptr->geometry->type != urdf::Geometry::BOX) {
          RCLCPP_ERROR(
            this->get_logger(),
            "Link %s uses a non-box geometry, only boxes are supportede. Skipping element",
            link->name.c_str());
          continue;
        }
    ...
    if (robot.getDepthEnabled()) {
      ...
      if (!hasNans(depthMsg)) {
        ...
        RCLCPP_WARN_SKIPFIRST_THROTTLE(
          get_logger(), *get_clock(), 1000, "NaNs present in outgoing depth message. Skipped.");
      }
    }
  }
  ...
  void publishFakeDVLData()
  {
    if (robot.getDVLEnabled() && robot.dvlTransformAvailable()) {
      ...
      if (!hasNans(dvlMsg)) {
        ...
        RCLCPP_WARN_SKIPFIRST_THROTTLE(
          get_logger(), *get_clock(), 1000, "NaNs present in outgoing DVL message. Skipped.");
      }
    }
  }
  ...
  void publishFakeIMUData()
  {
    if (robot.getIMUEnabled() && robot.imuTransformAvailable()) {
      ...
      if (!hasNans(imuMsg)) {
        ...
        RCLCPP_WARN_SKIPFIRST_THROTTLE(
          get_logger(), *get_clock(), 1000, "NaNs present in outgoing IMU message. Skipped.");
      }
    }
  }
  ...
  bool hasNans(const sensor_msgs::msg::Imu & imuMsg)
  {
    return hasNans(imuMsg.angular_velocity) || hasNans(imuMsg.angular_velocity_covariance) ||
           hasNans(imuMsg.linear_acceleration) || hasNans(imuMsg.linear_acceleration_covariance) ||
           hasNans(imuMsg.orientation) || hasNans(imuMsg.orientation_covariance);
  }
