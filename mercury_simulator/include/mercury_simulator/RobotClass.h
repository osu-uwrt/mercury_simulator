...
#include "mercury_simulator/Claw_Object.h"
...
struct ActiveBallastStates {
    bool exhaustState, pressureState, waterState;
};
...
private:
    ...
    double ballast_max_mass;     // Maximum mass increase from active ballast, computed from volume.
                                 // stored here (instead of volume) for efficiency. kg
    double ballast_in_flow_rate; // Volume of water (m^3) entering ballast per second while intaking
                                 // water
    double ballast_out_flow_rate; // Volume of water (m^3) exiting ballast per second while
                                  // expelling water
    ...
    geometry_msgs::msg::Transform safeTransform(
        std::string toFrame, std::string fromFrame, bool & transformFlag);
};
